...
#ifdef ARDUINO
    pX->file->seekSet(filepos); // in case I'm doing a 'retry' and I have to re-read part of the file
#else  // ARDUINO
...
/*
  ...
  */
  ...
  xx.file = sdfile;
  if(!xx.file->open(szFilename, O_CREAT | O_EXCL | O_WRITE))
  {
...
XMODEM xx;
  ...
  xx.file = sdfile;
  if(!xx.file->open(szFilename, O_READ))
  {
